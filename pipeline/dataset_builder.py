...
def build_dataset(applicants_path, vagas_path, prospect_path, output_path, negatives_ratio=1):
    ...
    for _, row in tqdm(prospect_df.iterrows(), total=len(prospect_df)):
        ...
        emb_app = np.array([applicant_dict[cod_app][f'embedding_{i}'] for i in range(300)]).reshape(1, -1)
        emb_vaga = np.array([vaga_dict[cod_vaga][f'embedding_{i}'] for i in range(300)]).reshape(1, -1)
    ...
    attempts = 0
    while len(dataset) < len(prospect_df) + num_negativos and attempts < 10 * num_negativos:
        app = np.random.choice(all_applicants)
        vaga = np.random.choice(all_vagas)

        if not ((prospect_df['codigo'] == app) & (prospect_df['codigo_vaga'] == vaga)).any():
            emb_app = np.array([applicant_dict[app][f'embedding_{i}'] for i in range(300)]).reshape(1, -1)
            emb_vaga = np.array([vaga_dict[vaga][f'embedding_{i}'] for i in range(300)]).reshape(1, -1)
            similarity = cosine_similarity(emb_app, emb_vaga)[0][0]
            dataset.append({'codigo_candidato': app, 'codigo_vaga': vaga, 'similarity': similarity, 'match': 0})
        attempts += 1
